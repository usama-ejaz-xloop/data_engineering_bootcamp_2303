...
class AI(Player):
    ...
    def placing_ships_on_the_field(self, size):
        ...
        def place_ship(fld, cur_fld, trn):
            current_ship_position = set(
                [place for place in range(len(fld)) if fld[place] == "&"]
            )
            forb_places = CheckSurround(fld).forbid_placement()
            forb_places_upd = [place for place in forb_places if cur_fld[place] == trn]
        ...
        while True:
            ...
            try:
                while attempts != randoms:
                    commands[random.choice(("w", "d", "s", "a", "r"))](
                        old_field, upd_field
                    )
    ...
    def shooting(self):
        ...
        wounded_ships = [
            deck
            for deck in range(len(self.opponent.field))
            if self.opponent.field[deck] == "x"
            and self.opponent.field[deck] not in self.ships_hit
        ]
        if len(wounded_ships) == 1:
            while True:
                shot = random.choice(list(AI.shooting_area(wounded_ships)))
            ...
            if self.opponent.field[wounded_ships[-1] - 1] == "x":
                while True:
                    shot = random.choice(
                        list(AI.horizontal_shooting_area(wounded_ships))
                    )
                    if self.opponent.field[shot] == self.opponent.turn:
                        self.opponent.field[shot] = "x"
                        break
                ...
                while True:
                    shot = random.choice(list(AI.upright_shooting_area(wounded_ships)))
                    if self.opponent.field[shot] == self.opponent.turn:
                        self.opponent.field[shot] = "x"
                        break
                    elif self.opponent.field[shot] is None:
            ...
            available_to_shoot = random.choice(
                [
                    pos
                    for pos in range(len(self.opponent.field))
                    if self.opponent.field[pos] != "o"
                ]
            )
    ...
    def shooting_area(ship_position):
        ...
        for place in ship_position:
            if place in Field.r_upper_corner:
                set_of_pos.update({place - 1}, {place + Field.num_of_lines})
                ...
                set_of_pos.update(
                    {place + 1}, {place - 1}, {place + Field.num_of_lines}
                )
    ...
    def horizontal_shooting_area(ship_position):
        ...
        for place in ship_position:
            if place in Field.r_upper_corner:
                ...
                set_of_pos.update({place + 1}, {place - 1})
            elif place in Field.bottom_border:
                set_of_pos.update({place + 1}, {place - 1})
            else:
                set_of_pos.update({place + 1}, {place - 1})
    ...
    def upright_shooting_area(ship_position):
        ...
        for place in ship_position:
            if place in Field.r_upper_corner:
                set_of_pos.update({place + Field.num_of_lines})
            elif place in Field.r_bottom_corner:
                set_of_pos.update({place - Field.num_of_lines})
            elif place in Field.l_upper_corner:
                set_of_pos.update({place + Field.num_of_lines})
                ...
                set_of_pos.update(
                    {place - Field.num_of_lines}, {place + Field.num_of_lines}
                )
            elif place in Field.left_border:
                set_of_pos.update(
                    {place + Field.num_of_lines}, {place - Field.num_of_lines}
